...
class FormatVisitor(object):
    ...
    def visit(self, node, tabs=0):
        ...
        method_decl_col = '\t' * (tabs + 1) + 'MethodDeclCol:\n' + '\n'.join(self.visit(method_decl, tabs + 1) for method_decl in node.method_decl_col)
    ...
    def visit(self, node, tabs=0):
        ...
        value = '\t' * (tabs + 1) + self.visit(node.value, tabs + 1)
        return f'{ans}\n{value}'
    ...
    def visit(self, node, tabs=0):
        ans = '\t' * tabs + f'\\__IfNode'
        conditions = '\t' * (tabs + 1) + 'Conditions:\n' + '\n'.join(self.visit(condition, tabs + 1) for condition in node.conditions)
        return f'{ans}\n{conditions}'
    ...
    def visit(self, node, tabs=0):
        ...
        var_decl = '\t' * (tabs + 1) + 'Declarations:\n' + '\n'.join(self.visit(decl, tabs + 1) for decl in node.var_decl)
        body = self.visit(node.body, tabs + 1)
        return f'{ans}\n{var_decl}\n{body}'
